...
export async function GET(req: Request) {
  try {
    ...
    if (expiring) {
      return NextResponse.json(enriched.filter(c => c.status === "expiring" || c.status === "expired"));
    }
...
export async function POST(req: Request) {
  try {
    ...
    if (!workspaceId || !title || !customer || !startDate || !expiryDate) {
      return NextResponse.json({ error: "workspaceId, title, customer, startDate, expiryDate are required" }, { status: 400 });
    }
