"use client";
// app/portal/page.tsx
// The Customer Portal — a separate page your customers visit to:
// - See their orders and track status
// - View and accept/decline quotes
// - Check invoices and payment status
// - Submit new order requests
//
// Access: yourapp.vercel.app/portal
// Later we'll add custom domains like: acmecorp.yourdomain.com

import { useState, useEffect } from "react";

// ── Colour palette — intentionally different from the main app ───────────────
// The portal is customer-facing so it feels clean, light, and trustworthy.
const P = {
  bg:          "#f7f8fc",
  surface:     "#ffffff",
  border:      "#e4e8f0",
...
interface PortalOrder {
  id: string; sku: string; items: number;
  value: number; status: OrderStatus; date: string; eta: string;
}
interface PortalQuote {
  id: string; quoteNumber: string; items: number;
  value: number; status: QuoteStatus; validUntil: string; date: string;
}
interface PortalInvoice {
  id: string; invoiceNumber: string;
  value: number; status: InvoiceStatus; due: string; date: string;
}

// ── Demo customers (simulate different logins) ───────────────────────────────
...
const SectionTitle = ({ children }: any) => (
  <div style={{ fontWeight: 800, fontSize: 15, color: P.text, marginBottom: 16 }}>
    {children}
  </div>
);

// ── Status badges ─────────────────────────────────────────────────────────────
const ORDER_STATUS_STYLE: Record<OrderStatus, any> = {
  Placed:    { color: P.blue,   bg: P.blueBg,   border: P.blueBorder   },
  Confirmed: { color: P.purple, bg: P.purpleBg,  border: P.purpleBorder },
  Picked:    { color: P.amber,  bg: P.amberBg,   border: P.amberBorder  },
  Shipped:   { color: P.blue,   bg: P.blueBg,    border: P.blueBorder   },
  Delivered: { color: P.green,  bg: P.greenBg,   border: P.greenBorder  },
};
const QUOTE_STATUS_STYLE: Record<QuoteStatus, any> = {
  pending:  { color: P.amber, bg: P.amberBg,  border: P.amberBorder, label: "Awaiting Review" },
  accepted: { color: P.green, bg: P.greenBg,  border: P.greenBorder, label: "Accepted"        },
  declined: { color: P.red,   bg: P.redBg,    border: P.redBorder,   label: "Declined"        },
  expired:  { color: P.muted, bg: "#f3f4f6",  border: P.border,      label: "Expired"         },
};
const INVOICE_STATUS_STYLE: Record<InvoiceStatus, any> = {
  unpaid:  { color: P.amber, bg: P.amberBg,  border: P.amberBorder, label: "Unpaid"      },
  paid:    { color: P.green, bg: P.greenBg,  border: P.greenBorder, label: "Paid"        },
  overdue: { color: P.red,   bg: P.redBg,    border: P.redBorder,   label: "Overdue"     },
  partial: { color: P.blue,  bg: P.blueBg,   border: P.blueBorder,  label: "Partial Pay" },
};

const Badge = ({ label, style }: { label: string; style: any }) => (
  <span style={{
...
export default function PortalPage() {
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 36 }}>
        <div style={{
          ...
          fontSize: 22, boxShadow: "0 4px 18px #3b6fd433",
        }}>⚡</div>
        <div>
      ...
      <div style={{
        ...
        <h2 style={{ fontSize: 20, fontWeight: 800, color: P.text, marginBottom: 6 }}>
          Sign in to your portal
        </h2>
        <p style={{ fontSize: 13, color: P.muted, marginBottom: 24 }}>
          Your supplier shared an access code with you. Enter it below.
        </p>

        {/* Email */}
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: P.muted, marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.05em" }}>
            Your Email
          </label>
          <input
            value={email}
            ...
            style={{
              ...
              background: P.bg, border: `1px solid ${loginErr ? P.red : P.border}`,
        ...
        {loginErr && (
          <div style={{
            ...
            background: P.redBg, border: `1px solid ${P.redBorder}`,
            borderRadius: 8, fontSize: 13, color: P.red,
          }}>{loginErr}</div>
        )}
        ...
        <button
          ...
          style={{
            ...
            background: email && code
              ? "linear-gradient(135deg,#3b6fd4,#6d28d9)"
              : P.border,
            border: "none", borderRadius: 10,
            color: email && code ? "#fff" : P.muted,
            fontSize: 15, fontWeight: 700, cursor: email && code ? "pointer" : "not-allowed",
          }}
        >
        ...
        <div style={{
          ...
          background: P.blueBg, border: `1px solid ${P.blueBorder}`,
          borderRadius: 10,
        }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: P.blue, marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.05em" }}>
            🎯 Demo Credentials
          </div>
          {[
            ...
            <button
              key={d.email}
              onClick={() => { setEmail(d.email); setCode(d.code); setLoginErr(""); }}
              style={{
                display: "block", width: "100%", textAlign: "left",
                padding: "8px 10px", marginBottom: 6, borderRadius: 7,
                background: P.surface, border: `1px solid ${P.blueBorder}`,
  ...
  return (
    <div style={{
      minHeight: "100vh", background: P.bg,
      fontFamily: "'DM Sans','Segoe UI',system-ui,sans-serif", color: P.text,
    }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {pendingQuotes > 0 && (
            <button onClick={() => setTab("quotes")} style={{
              ...
              background: P.amberBg, border: `1px solid ${P.amberBorder}`,
              color: P.amber, fontSize: 11, fontWeight: 700,
            }}>
              ⏳ {pendingQuotes} quote{pendingQuotes > 1 ? "s" : ""} awaiting review
            </button>
          )}
          {overdueInv > 0 && (
            <button onClick={() => setTab("invoices")} style={{
              ...
              padding: "4px 12px", borderRadius: 999, cursor: "pointer",
              background: P.redBg, border: `1px solid ${P.redBorder}`,
              color: P.red, fontSize: 11, fontWeight: 700,
            }}>
              ⚠️ {overdueInv} overdue invoice{overdueInv > 1 ? "s" : ""}
            </button>
          )}
          <div style={{ fontSize: 13, color: P.muted }}>
            Hi, <strong style={{ color: P.text }}>{customer.name.split(" ")[0]}</strong>
          </div>
          <button
            onClick={logout}
            style={{
        ...
        {TABS.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              ...
              border: "none", borderBottom: active ? `2px solid ${P.blue}` : "2px solid transparent",
      ...
      <div style={{ padding: "28px", maxWidth: 900, margin: "0 auto" }}>
              ...
              return (
                <Card key={order.id}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                    <div>
                      <div style={{ fontWeight: 800, fontSize: 15, color: P.text, marginBottom: 3 }}>
                        {order.sku}
                      </div>
                      <div style={{ fontSize: 13, color: P.muted }}>
                        {order.items} items · Ordered {order.date}
                      </div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontWeight: 800, fontSize: 18, color: P.text }}>{fmtMoney(order.value)}</div>
                      <Badge label={order.status} style={s} />
                    </div>
                  </div>
                        ...
                        <div key={stage} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
                          <div style={{ display: "flex", alignItems: "center", width: "100%" }}>
                            {i > 0 && (
                              <div style={{
                                flex: 1, height: 3,
                                background: done ? P.blue : P.border,
                                transition: "background 0.3s",
                              }} />
                            )}
                            <div style={{
                              width: 28, height: 28, borderRadius: "50%", flexShrink: 0,
                              background: done ? P.blue : P.surface,
                              border: `2px solid ${done ? P.blue : P.border}`,
                              display: "flex", alignItems: "center", justifyContent: "center",
                              fontSize: 11, fontWeight: 800,
                              color: done ? "#fff" : P.subtle,
                              boxShadow: current ? `0 0 0 4px ${P.blueBg}` : "none",
                              transition: "all 0.3s",
                            }}>
                              {done && !current ? "✓" : i + 1}
                            </div>
                            {i < ORDER_STAGES.length - 1 && (
                              <div style={{
                                flex: 1, height: 3,
                                background: i < stageIdx ? P.blue : P.border,
                                transition: "background 0.3s",
                              }} />
                            )}
                          </div>
                          <div style={{
                            fontSize: 10, marginTop: 6, fontWeight: current ? 700 : 400,
                            color: current ? P.blue : done ? P.muted : P.subtle,
                            textAlign: "center",
                          }}>{stage}</div>
                        </div>
                      );
                  ...
                  {order.status !== "Delivered" && (
                    <div style={{
                      ...
                      background: P.blueBg, border: `1px solid ${P.blueBorder}`,
                      borderRadius: 8, fontSize: 12, color: P.blue,
                    }}>
        ...
        {tab === "quotes" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {quotes.map(q => {
              ...
              return (
                <Card key={q.id} style={{
                  border: pending ? `1px solid ${P.amberBorder}` : `1px solid ${P.border}`,
                  background: pending ? P.amberBg : P.surface,
                }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: pending ? 16 : 0 }}>
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                        <span style={{ fontWeight: 800, fontSize: 15, fontFamily: "monospace", color: P.text }}>
                          {q.quoteNumber}
                        </span>
                        <Badge label={s.label} style={s} />
                      </div>
                      <div style={{ fontSize: 13, color: P.muted }}>
                        {q.items} item{q.items !== 1 ? "s" : ""} · Sent {q.date} · Valid until {q.validUntil}
                      </div>
                    </div>
                  ...
                  {/* Accept / Decline buttons — only for pending quotes */}
                  {pending && (
                    <div style={{ display: "flex", gap: 10 }}>
                      <button
                        ...
                        style={{
                          flex: 1, padding: "11px",
                          background: "linear-gradient(135deg,#1a7f5a,#28a770)",
                          border: "none", borderRadius: 9,
                          color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer",
                          boxShadow: "0 4px 14px #1a7f5a33",
                        }}
                      >✓ Accept Quote</button>
                      <button
                        ...
                        style={{
                          flex: 1, padding: "11px",
                          background: P.surface, border: `1px solid ${P.redBorder}`,
                          borderRadius: 9, color: P.red,
            ...
            {(() => {
              ...
              return outstanding > 0 ? (
                <div style={{
                  ...
                  <span style={{ fontSize: 13, color: P.amber, fontWeight: 700 }}>
                    💳 Total outstanding balance
                  </span>
                  <span style={{ fontSize: 20, fontWeight: 800, color: P.amber }}>
            ...
            {customer.invoices.map(inv => {
              ...
              return (
                <Card key={inv.id} style={{
                  border: inv.status === "overdue" ? `1px solid ${P.redBorder}` : `1px solid ${P.border}`,
                }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      ...
                      <div style={{ fontSize: 13, color: P.muted }}>
                        Issued {inv.date} · Due <strong style={{ color: inv.status === "overdue" ? P.red : P.text }}>{inv.due}</strong>
                      </div>
                    </div>
                    <div style={{ textAlign: "right" }}>
        ...
        {tab === "request" && (
          <div style={{ maxWidth: 560 }}>
            <SectionTitle>Submit a New Order Request</SectionTitle>

            {reqSent ? (
              <Card style={{ textAlign: "center", padding: "48px 24px" }}>
                <div style={{ fontSize: 48, marginBottom: 16 }}>🎉</div>
                <h3 style={{ fontSize: 20, fontWeight: 800, color: P.text, marginBottom: 8 }}>
                  Request Submitted!
                </h3>
                <p style={{ color: P.muted, fontSize: 14, marginBottom: 24, lineHeight: 1.6 }}>
                  Your supplier has received your request and will send you a quote within 1–2 business days.
                  You'll see it appear in the <strong>Quotes</strong> tab once it's ready.
                </p>
                <button
                  onClick={() => { setReqSent(false); setReqSku(""); setReqQty(""); setReqNotes(""); setReqDeadline(""); }}
                  style={{
                    padding: "11px 24px", borderRadius: 9,
                    background: P.blueBg, border: `1px solid ${P.blueBorder}`,
                    color: P.blue, fontSize: 13, fontWeight: 700, cursor: "pointer",
                  }}
              ...
              <Card>
                <p style={{ fontSize: 13, color: P.muted, marginBottom: 22, lineHeight: 1.6 }}>
                  Tell your supplier what you need. They'll review it and send back a quote — usually within 1–2 business days.
                </p>

                {[
                  { label: "SKU / Product Name *", placeholder: "e.g. SKU-4821 or 'Industrial bolts M10'", val: reqSku,      set: setReqSku      },
                  { label: "Quantity Needed *",     placeholder: "e.g. 500 units",                          val: reqQty,      set: setReqQty      },
                ...
                <button
                  ...
                  disabled={!reqSku.trim() || !reqQty.trim()}
                  style={{
                    width: "100%", padding: "13px",
                    background: reqSku && reqQty
                      ? "linear-gradient(135deg,#3b6fd4,#6d28d9)"
                      : P.border,
                    border: "none", borderRadius: 10,
                    color: reqSku && reqQty ? "#fff" : P.muted,
                    fontSize: 15, fontWeight: 700,
                    cursor: reqSku && reqQty ? "pointer" : "not-allowed",
                  }}
                >
                  ✍️ Submit Request
                </button>
