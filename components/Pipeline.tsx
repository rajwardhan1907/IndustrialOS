...
import {
  Upload, CheckCircle, XCircle, AlertTriangle, FileText, RefreshCw,
  Package, Users, Factory, ChevronDown, ChevronRight,
} from "lucide-react";
import { C, fmt } from "@/lib/utils";
import { Card, SectionTitle } from "./Dashboard";

// ── Inventory types (UNCHANGED) ───────────────────────────────────────────────
interface ParsedRow {
  sku:          string;
  name:         string;
  category?:    string;
...
interface GenericUploadState {
  ...
  inserted:     number;
  errors:       number;
  errorDetails: { row: number; identifier: string; msg: string }[];
  elapsed?:     string;
  message?:     string;
}
...
// ── Inventory CSV parser (UNCHANGED) ─────────────────────────────────────────
...
// ── Inventory JSON parser (UNCHANGED) ────────────────────────────────────────
...
// ── Inventory batch sender (UNCHANGED) ───────────────────────────────────────
const BATCH = 100;
...
// ── Inventory template download (UNCHANGED) ──────────────────────────────────
...
const CUSTOMER_ALIASES: Record<string, string> = {
  ...
  "contact name": "contactName", "contact person": "contactName",
  "email": "email", "email address": "email",
  "phone": "phone", "phone number": "phone", "tel": "phone",
  "country": "country",
  "industry": "industry", "sector": "industry",
  "creditlimit": "creditLimit", "credit limit": "creditLimit",
  "balancedue": "balanceDue", "balance": "balanceDue", "balance due": "balanceDue",
  "totalspend": "totalSpend", "total spend": "totalSpend", "lifetime spend": "totalSpend",
  "status": "status",
  "notes": "notes", "comments": "notes",
  "portalcode": "portalCode", "portal code": "portalCode", "portal id": "portalCode",
};

const CUSTOMER_TEMPLATE =
  "name,contactName,email,phone,country,industry,creditLimit,balanceDue,status,notes,portalCode\n" +
  "Apex Fabrication Inc,Derek Holt,derek@apexfab.com,312-555-0101,USA,Manufacturing,250000,0,active,Key account,VIS001\n" +
  "Gulf Coast Petrotech,Omar Castellano,omar@gcpetro.com,713-555-0303,USA,Oil & Gas,500000,41000,active,,VIS002";

const SUPPLIER_ALIASES: Record<string, string> = {
  "name": "name", "supplier name": "name", "company": "name",
  "contactname": "contactName", "contact": "contactName",
  "contact name": "contactName", "contact person": "contactName",
  "email": "email", "email address": "email",
  "phone": "phone", "tel": "phone",
  "country": "country",
  "category": "category", "type": "category", "supplier type": "category",
  "status": "status",
  "paymentterms": "paymentTerms", "payment terms": "paymentTerms", "terms": "paymentTerms",
  "leadtimedays": "leadTimeDays", "lead time": "leadTimeDays", "lead time days": "leadTimeDays",
  "rating": "rating",
  "notes": "notes", "comments": "notes",
};

const SUPPLIER_TEMPLATE =
  "name,contactName,email,phone,country,category,paymentTerms,leadTimeDays,rating,notes\n" +
  "Parker Hannifin Corp,James Whitfield,james@parker.com,216-896-3000,USA,hydraulics,Net 30,10,5,Tier-1 preferred\n" +
  "SMC Corporation,Kenji Tanaka,k.tanaka@smcusa.com,317-899-4440,USA,pneumatics,Net 30,7,5,Fast shipping";
...
function SectionUploader({
  ...
  const parseGenericCSV = (text: string): Record<string, string>[] => {
    const lines = text.trim().split(/\r?\n/);
    if (lines.length < 2) return [];

    const rawHeaders = lines[0].split(",").map(h => h.trim().replace(/^"|"$/g, "").toLowerCase());
    const headers = rawHeaders.map(h => fieldAliases[h] ?? null);
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const hasRequired = requiredFields.every(f => row[f]?.trim());
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    if (!workspaceId) {
      setState({ status: "error", fileName: file.name, totalRows: 0, sentRows: 0, inserted: 0, errors: 0, errorDetails: [], message: "No workspace found. Please log in first." });
      return;
    }

    const ext = file.name.split(".").pop()?.toLowerCase();
    if (ext === "xlsx" || ext === "xls") {
      setState({ status: "error", fileName: file.name, totalRows: 0, sentRows: 0, inserted: 0, errors: 0, errorDetails: [], message: "Excel upload: please export your sheet as CSV first (File → Save As → CSV), then upload the .csv file here." });
      return;
    }

    setState({ status: "parsing", fileName: file.name, totalRows: 0, sentRows: 0, inserted: 0, errors: 0, errorDetails: [] });

    const text = await file.text();
    const rows = parseGenericCSV(text);

    if (rows.length === 0) {
      setState({ status: "error", fileName: file.name, totalRows: 0, sentRows: 0, inserted: 0, errors: 0, errorDetails: [], message: `No valid rows found. Make sure your file has a '${requiredFields.join("', '")}' column. Download the template to see the expected format.` });
      return;
    }
    ...
    const allErrors: { row: number; identifier: string; msg: string }[] = [];
      ...
      await Promise.all(chunk.map(async (row, idx) => {
        ...
        try {
          ...
          if (res.ok) {
            ...
            const isPortalDupe = res.status === 500 &&
              (lower.includes("portalcode") || lower.includes("portal_code")) &&
              (lower.includes("unique") || lower.includes("duplicate"));
            errors++;
            allErrors.push({
              row: rowNum,
              identifier: row[requiredFields[0]] || "",
              msg: isPortalDupe ? "Duplicate portal code — row skipped" : `HTTP ${res.status}`,
            });
          }
        } catch {
          errors++;
          allErrors.push({ row: rowNum, identifier: row[requiredFields[0]] || "", msg: "Network error" });
        }
      }));

      setState(s => s ? { ...s, sentRows: offset + chunk.length, inserted, errors, errorDetails: [...allErrors] } : s);
    }
  ...
  }, [apiEndpoint, batchSize, onSuccess]);  // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        <div style={{ width: 56, height: 56, background: C.blueBg, border: `1px solid ${C.blueBorder}`, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <Upload size={22} color={C.blue} />
        </div>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontWeight: 800, fontSize: 16, color: C.text }}>Bulk {entityName} Upload</div>
          <div style={{ color: C.muted, fontSize: 13, marginTop: 4 }}>Drop a CSV file here, or click to browse</div>
          <div style={{ color: C.subtle, fontSize: 12, marginTop: 2 }}>Sends {batchSize} rows per batch · inserts directly to database</div>
        </div>
      </div>
          ...
          <Card>
            ...
            {p.status === "done" && (
              <div style={{ marginTop: 14, padding: "10px 14px", background: C.greenBg, border: `1px solid ${C.greenBorder}`, borderRadius: 10, fontSize: 13, color: C.green, display: "flex", alignItems: "center", gap: 8 }}>
                <CheckCircle size={13} />
                Done — {fmt(p.inserted)} {entityName.toLowerCase()} imported in {p.elapsed}s.
              </div>
            )}
          </Card>
              ...
              <div style={{ maxHeight: 240, overflowY: "auto" }}>
                {p.errorDetails.map((e, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "7px 0", borderBottom: `1px solid ${C.border}`, fontSize: 12 }}>
                    ...
                    <span style={{ fontFamily: "monospace", color: C.text }}>{e.identifier || "(no name)"}</span>
...
export default function Pipeline() {
  // Inventory upload state (UNCHANGED)
  const [state,    setState]    = useState<PipeState | null>(null);
  const [mode,     setMode]     = useState<"upsert" | "replace">("upsert");
  const [dragging, setDragging] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);

  // Accordion + count state
  const [openSection, setOpenSection] = useState<string | null>("inventory");
  const [counts,      setCounts]      = useState<ImportCounts>({ inventory: null, customers: null, suppliers: null });
  ...
  // Fetch record counts on mount
  useEffect(() => {
    const wid = getWorkspaceId();
    if (!wid) return;
    const fetchCount = async (url: string, key: keyof ImportCounts) => {
      try {
        const res = await fetch(`${url}?workspaceId=${wid}`);
        if (res.ok) {
          const data = await res.json();
          const arr = Array.isArray(data) ? data : [];
          setCounts(c => ({ ...c, [key]: arr.length }));
        }
      } catch {}
    };
    fetchCount("/api/inventory", "inventory");
    fetchCount("/api/customers", "customers");
    fetchCount("/api/suppliers", "suppliers");
  }, []);

  // Refresh a single count after import
  const refreshCount = useCallback(async (key: keyof ImportCounts) => {
    const wid = getWorkspaceId();
    if (!wid) return;
    const endpoints: Record<keyof ImportCounts, string> = {
      inventory: "/api/inventory",
      customers: "/api/customers",
      suppliers: "/api/suppliers",
    };
    try {
      const res = await fetch(`${endpoints[key]}?workspaceId=${wid}`);
      if (res.ok) {
        const data = await res.json();
        const arr = Array.isArray(data) ? data : [];
        setCounts(c => ({ ...c, [key]: arr.length }));
  ...
  // Refresh inventory count after successful inventory import
  useEffect(() => {
    if (state?.status === "done" && (state.inserted > 0 || state.updated > 0)) {
      refreshCount("inventory");
    }
  }, [state?.status]); // eslint-disable-line react-hooks/exhaustive-deps

  // Inventory file processing (UNCHANGED)
  const processFile = useCallback(async (file: File) => {
    const workspaceId = getWorkspaceId();
    if (!workspaceId) {
  ...
  const countBadge = (count: number | null) => {
    if (count === null) return (
      <span style={{ padding: "2px 10px", borderRadius: 999, fontSize: 11, background: C.surface, border: `1px solid ${C.border}`, color: C.subtle }}>...</span>
    );
    if (count === 0) return (
      <span style={{ padding: "2px 10px", borderRadius: 999, fontSize: 11, background: C.surface, border: `1px solid ${C.border}`, color: C.muted }}>0 records</span>
    );
    return (
      <span style={{ padding: "2px 10px", borderRadius: 999, fontSize: 11, background: C.greenBg, border: `1px solid ${C.greenBorder}`, color: C.green, fontWeight: 700 }}>
        {fmt(count)} records
      </span>
    );
  };
  ...
  // Section header
  const renderSectionHeader = (id: string, Icon: any, title: string, desc: string, count: number | null) => {
    ...
    return (
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 36, height: 36, background: C.blueBg, border: `1px solid ${C.blueBorder}`, borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      {/* ── Page header ── */}
      <div>
        <div style={{ fontSize: 20, fontWeight: 800, color: C.text }}>Data Import</div>
        <div style={{ fontSize: 13, color: C.muted, marginTop: 4 }}>Bulk import master data from CSV. Inventory · Customers · Suppliers</div>
      </div>

      {/* ── Inventory / SKUs section ── */}
      <div>
        {renderSectionHeader("inventory", Package, "Inventory / SKUs", "Bulk import products, stock levels, and warehouse locations", counts.inventory)}
        {openSection === "inventory" && (
          <div style={{ border: `1px solid ${C.border}`, borderTop: "none", borderRadius: "0 0 12px 12px", padding: 20, background: C.bg }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              ...
              >
                ...
                {/* Import mode selector (UNCHANGED) */}
              ...
              {/* Progress / result card (UNCHANGED) */}
              {p && p.status !== "idle" && (
                <>
                  <Card>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              ...
              {/* Format guide (UNCHANGED — shown when nothing is loaded) */}
              {!p && (
                <Card>
      ...
      {/* ── Customers section ── */}
      <div>
        {renderSectionHeader("customers", Users, "Customers", "Bulk import customer accounts, contacts, and credit limits", counts.customers)}
        {openSection === "customers" && (
          <div style={{ border: `1px solid ${C.border}`, borderTop: "none", borderRadius: "0 0 12px 12px", padding: 20, background: C.bg }}>
            <SectionUploader
              ...
              onSuccess={() => refreshCount("customers")}
            />
          </div>
        )}
      </div>
      ...
      {/* ── Suppliers section ── */}
      <div>
        {renderSectionHeader("suppliers", Factory, "Suppliers", "Bulk import supplier contacts, categories, and lead times", counts.suppliers)}
        {openSection === "suppliers" && (
          <div style={{ border: `1px solid ${C.border}`, borderTop: "none", borderRadius: "0 0 12px 12px", padding: 20, background: C.bg }}>
            <SectionUploader
              ...
              onSuccess={() => refreshCount("suppliers")}
            />
          </div>
        )}
      </div>
