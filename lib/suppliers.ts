  ...
  return [
    {
      ...
      expectedDate:future(7), notes:"Urgent — for Q2 production run.", createdAt:ago(5),
    },
    {
      ...
      expectedDate:ago(3).split("T")[0], notes:"", createdAt:ago(30),
    },
    {
      ...
      expectedDate:future(14), notes:"", createdAt:ago(1),
    },
  ];
}
...
export function loadPOs(): PurchaseOrder[] {
  ...
  try {
    ...
    if (!raw) {
      const demo = seedPurchaseOrders(loadSuppliers());
...
// ── API: create purchase order in DB ─────────────────────────────────────────
...
// ── API: update purchase order in DB ─────────────────────────────────────────
