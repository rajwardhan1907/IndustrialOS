// lib/inventory.ts
// Shared inventory store (localStorage).
...
export interface InventoryItem {
  ...
  name:           string;
  category:       string;
  stockLevel:     number;   // current units in stock
  reorderPoint:   number;   // alert when stock falls below this
  reorderQty:     number;   // how many to reorder
  unitCost:       number;   // cost per unit in $
  warehouse:      string;   // e.g. "Warehouse A"
  zone:           WarehouseZone;
